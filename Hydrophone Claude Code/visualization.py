...
def update_comment_markers():
    ...
    for comment in sorted_comments:
        ...
        state.ax_comment_timeline.text(text_x, text_y, text,
                                     ha='center', va='center',
                                     fontsize=11, color=text_color,  # Use appropriate text color
                                     weight='bold',
                                     clip_on=True)
...
def display_selected_comment():
    """Display the selected comment in the comment display window"""
    # Check if display window exists
    if not hasattr(state, 'ax_comment_display') or state.ax_comment_display is None:
        return
    ...
    # If no comment selected, show placeholder
    if state.selected_comment_id is None:
    ...
    if selected_comment:
        ...
        lines = []
        lines.append(f"Comment: {selected_comment['text']}")
        lines.append("")  # blank line
        
        # Convert indices to time format if possible
        if hasattr(state, 'time_labels_all') and state.time_labels_all:
            start_time = state.time_labels_all[selected_comment['start_idx']][:5] if selected_comment['start_idx'] < len(state.time_labels_all) else str(selected_comment['start_idx'])
            end_time = state.time_labels_all[selected_comment['end_idx']][:5] if selected_comment['end_idx'] < len(state.time_labels_all) else str(selected_comment['end_idx'])
            lines.append(f"Time: {start_time} - {end_time}")
        else:
            lines.append(f"Range: {selected_comment['start_idx']} - {selected_comment['end_idx']}")
        
        lines.append("")  # blank line
        
        if selected_comment.get('user_notes'):
            lines.append("Notes:")
            # Word wrap the notes
            words = selected_comment['user_notes'].split()
            current_line = ""
            for word in words:
        ...
        # Display the text - increased font size for better readability
        text = "\n".join(lines)
        state.ax_comment_display.text(0.02, 0.98, text,
                                    transform=state.ax_comment_display.transAxes,
                                    fontsize=10,
...
def create_audio_timeline_axis():
    ...
    if hasattr(state, 'ax_comment_timeline') and state.ax_comment_timeline is not None:
        ...
        timeline_bottom = comment_pos.y0 - 0.03 + 0.005  # Small overlap
    else:
        # Fallback to below spectrogram directly
        timeline_bottom = spec_pos.y0 - 0.10
    
    # Create timeline axis below the comment timeline
